...
public class Maze {
  ...
  public Maze(int horizontalSize, int verticalSize) {
	...
	//my array store the random order walls.At randomWalls[][0],0-horizontal wall,1-vertical wall;
	//randomWalls[][1]¡¢randomWalls[][2] represents wall's x and y.	
	randomWalls = new int[2 * horiz * vert - horiz - vert][3];
	int count = 0;
    ...
    if (vert > 1) {
      ...
      for (j = 0; j < vert - 1; j++) {
        for (i = 0; i < horiz; i++) {
          hWalls[i][j] = true;
		  randomWalls[count] = new int[]{0, i, j};      //Initialize randomWalls
		  //randomWalls[count][0] = 0;
		  //randomWalls[count][1] = i;
		  //randomWalls[count][2] = j;
		  count++;
        }
      }
    }
    ...
    if (horiz > 1) {
      ...
      for (i = 0; i < horiz - 1; i++) {
        for (j = 0; j < vert; j++) {
          vWalls[i][j] = true;
		  //randomWalls[count] = {1, i, j};      //Initialize randomWalls, count == randomWalls.length()
		  randomWalls[count][0] = 1;
		  randomWalls[count][1] = i;
		  randomWalls[count][2] = j;
		  count++;
        }
	...
	//make randomWalls random order
	while (count > 1) {
		int rand = randInt(count);
		int[] temp = randomWalls[count - 1];
		randomWalls[count - 1] = randomWalls[rand];
		randomWalls[rand] = temp;
		count--;
	}
	
	//union cells and break walls to make maze
	for (i = 0; i < randomWalls.length; i++) {
		int hv    = randomWalls[i][0];
		int xWall = randomWalls[i][1];
		int yWall = randomWalls[i][2];
		int cellNum1 = yWall * horiz + xWall;     //index of wall's first cell in cells
		int cellNum2;
		
		if (hv == 0) {
			//wall is horizontal
			cellNum2 = cellNum1 + horiz;
		} else {
			cellNum2 = cellNum1 + 1;
		}
		int root1 = cells.find(cellNum1);
		int root2 = cells.find(cellNum2);
		if (root1 != root2) {
			cells.union(root1, root2);
			if (hv == 0) {
				hWalls[xWall][yWall] = false;
			} else {
				vWalls[xWall][yWall] = false;
			}
		}
	}
	...
	// Use DFS make maze.
	// at every cell in visit,test if its neighbours are visited,if not,break walls and visit them in random order.
