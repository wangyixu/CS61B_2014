...
public class HashTableChained implements Dictionary {
  ...
  public HashTableChained(int sizeEstimate) {
    // Your solution here.
	int buckets = makePrimerSize(sizeEstimate);
	table = new SList[buckets];
	numOfEntries = new int[buckets];
  }
  
  private int makePrimerSize(int sizeEstimate) {
	int buckets = 0;
	for (int i = ((int) sizeEstimate / 0.72); ; i++) {
		if (isPrimer(i)) {
			buckets = i;
			break;
		}
  ...
  public HashTableChained() {
    // Your solution here.
	table = new SList[101];            //modified for resize
	numOfEntries = new int[101];
  }
  ...
  public void collisionsInfo() {
    // Your solution here.
	int collisionsNum = 0;
	for (int i = 0; i < numOfEntries.length; i++) {
		System.out.print(numOfEntries[i] + " ");
		if (numOfEntries[i] > 1) {
			collisionsNum++;
		}
	}
	...
	System.out.print("In this HashTableChained, collisions number is  " + collisionsNum + " ,collision rate is " + 
		collisionsNum + "/" + numOfEntries.length + "(about " + (collisionsNum + 0.0) / numOfEntries.length + ").");
  }
	...
	*/
	public void resizeTrial() {
		...
		double loadFactor = loadSize / table.length;
		if (loadFactor > 0.72) {
			int num = makePrimerSize(loadSize * 2);
			int newTable
		}
	}
