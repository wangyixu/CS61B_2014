...
public class UDGraph
{
	...
	for (int i = 0; i < vertices; i++) {
		for (int j = 0; j < vertices; j++) {
			for (int k = 0; k < vertices; k++) {
				if ((adjMatrix[i][j] == true && adjMatrix[j][k] == true)) {     //v-w-v doesn't construct edge
  ...
  public UDGraph paths(int length) {
    UDGraph newGraph = new UDGraph(vertices);
    // Put your answer to Part II here.       length is >= 2
	if (length == 2) {
		...
		for (int i = 0; i < vertices; i++) {
			for (int j = 0; j < vertices; j++) {
				for (int k = 0; k < vertices; k++) {
					if (tempGraph.hasEdge(i, j) && this.hasEdge(j, k)) {      //edge's length is (length - 1) +  1.
						newGraph.addEdge(i, k);
					}
				}
			}
