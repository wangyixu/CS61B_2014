...
class SibTreeNode extends TreeNode {
  ...
  public void insertChild(Object item, int c) throws InvalidNodeException {
	...
	if (isValidNode()) {
		...
		if (firstChild == null || c <= 1) {           //add to the firstChild. 
			newNode.nextSibling = firstChild;
			firstChild = newNode;
		} else {
			SibTreeNode preNode = firstChild;
			int index = 2;
			while (preNode.nextSibling != null && index < c) {
				preNode = preNode.nextSibling;
				index++;
			}
			newNode.nextSibling = preNode.nextSibling;
  ...
  public void removeLeaf() throws InvalidNodeException {
	...
	if (isValidNode()) {
		if (children() == 0) {
			...
			if (parent != null) {             //不是唯一的根节点
				if (parent.firstChild == this) {
					parent.firstChild = nextSibling;
				} else {
					SibTreeNode preNode = parent.firstChild;
					while (preNode.nextSibling != this) {
						preNode = preNode.nextSibling;
					}
					preNode.nextSibling = nextSibling;
				}
				nextSibling = null;
				parent = null;
			}                
